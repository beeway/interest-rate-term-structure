...
library("readxl"); dm=read_excel("1945.xlsx")
...
attach(mc)
mc$Shape[U==1]="U"; mc$Shape[H==1]="H"; mc$Shape[F==1]="F"; mc$Shape[B==1]="B"; mc$Shape[D==1]="D"
# mc$ms[u==1]="u"; mc$ms[h==1]="h"; mc$ms[f==1]="f"; mc$ms[b==1]="b"; mc$ms[d==1]="d"
table(mc$Shape)

# creat a shape numerical indicator ns
mc$ns[U==1]="1"; mc$ns[H==1]="2"; mc$ns[D==1]="3"; mc$ns[B==1]="4"; mc$ns[F==1]="5"; 
# mc$ns[u==1]="1"; mc$ns[h==1]="2"; mc$ns[f==1]="5"; mc$ns[b==1]="4"; mc$ns[d==1]="3"
detach(mc)

write.csv(mc, file = "mshapes.csv")
...
g + geom_point(aes(col=ms), size=1, show.legend=T) +
  labs(y="Yield Curve Shape Dots", x="") 


# graph evolution as of 202012
md=read.csv("mshapes.csv")
...
ms=md$ms; ns=as.character(md$ns) 
...
g + geom_jitter(aes(col=ms), size=3) + theme_bw() +
  scale_x_continuous(breaks = seq(1945,2020, by = 3)) +
...
# graph evolution as of 202003
# create a year vector for wide data ggplot visulization so that each year has a column
year=c(1945:2019); year=rep(year, each=12); year=c(year, 2020, 2020, 2020)
month=c(1:12) ; month=rep(month,75); month=c(month,1,2,3)

wmc=data.frame(year, month, mc)

g=ggplot(mc,aes(year,ns))
g + geom_point(aes(col=Shape),size=3) + labs(y="Yield Curve Shape Dots", x="") 

g + geom_jitter(aes(col=Shape), size=3) + theme_bw() +
  scale_x_continuous(breaks = seq(1945,2020, by = 3)) +
  annotate("rect", xmin=1945.25, xmax=1945.83, ymin = 0, ymax = 6, alpha = .5) +
  annotate("rect", xmin=1949.00, xmax=1949.83, ymin = 0, ymax = 6, alpha = .5) +
  annotate("rect", xmin=1953.67, xmax=1954.42, ymin = 0, ymax = 6, alpha = .5) +
...
#  annotate("rect", xmin=1923.5, xmax=1924.7, ymin = 0, ymax = 6, alpha = .5) +
...
#  annotate("rect", xmin=1937.5, xmax=1938.5, ymin = 0, ymax = 6, alpha = .5) +
...
g + geom_point(aes(col=ms),size=2, show.legend=T) +
  ...
  g + geom_point(aes(col=month, size=ms)) +
  labs(y="Yield Curve Shape Dots", x="") 

g + geom_point(aes(col=month), size=1, show.legend=T) +
  labs(y="Yield Curve Shape Dots", x="") 
