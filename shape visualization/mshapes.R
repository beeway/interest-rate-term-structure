...
library("readxl"); dm=read_excel("mshapes.xlsx")
...
uy=matrix(NA,nrow(uyc), 11); for (j in 1: nrow(uyc)) {uy[j,]=as.numeric(uyc[j,1:11])}
plot(xm, uy[1,1:11], type='o', lty=1, lwd=1, col=1, ylab="Treasury Yield (%)",xlab="Maturity (Years)", xlim=c(0,30),ylim=c(0,17))
opt=c(2:nrow(uyc)); for(i in 2:nrow(uyc)){lines(xm,uy[i,], lwd=1, type="b", lty=opt[i], col=opt[i]) }

hy=matrix(NA,nrow(hyc), 11); for (j in 1: nrow(hyc)) {hy[j,]=as.numeric(hyc[j,1:11])}
plot(xm, hy[1,1:11], type="o", lwd=1, lty=1, col=1, ylab="Treasury Yield (%)",xlab="Maturity (Years)", xlim=c(0,30),ylim=c(0,17))
opt=c(2:nrow(hyc)); for(i in 2:nrow(hyc)){lines(xm,hy[i,1:11], lwd=1, type="b", lty=opt[i], col=opt[i]) }

fy=matrix(NA,nrow(fyc), 11); for (j in 1: nrow(fyc)) {fy[j,]=as.numeric(fyc[j,1:11 ])}
plot(xm,fy[1, ], type='o', lwd=1, col=1, ylab="Treasury Yield (%)",xlab="Maturity (Years)", xlim=c(0,30),ylim=c(0,17))
opt=c(2:nrow(fyc)); for(i in 2:nrow(fyc)){lines(xm,fy[i,], lwd=1, type="b", lty=opt[i], col=opt[i]) }

by=matrix(NA,nrow(byc), 11); for (j in 1: nrow(byc)) {by[j,]=as.numeric(byc[j,1:11])}
plot(xm, by[1, ], type='o', lty=1, lwd=1, col=1, ylab="Treasury Yield (%)",xlab="Maturity (Years)", xlim=c(0,30),ylim=c(0,17))
opt=c(2:nrow(byc)); for(i in 2:nrow(byc)){lines(xm, by[i,], lwd=1, type="b",lty=opt[i], col=opt[i]) }

dy=matrix(NA,nrow(dyc), 11); for (j in 1: nrow(dyc)) {dy[j,]=as.numeric(dyc[j,1:11])}
plot(xm, dy[1, ], type="o", lty=1, lwd=1, col=1, ylab="Treasury Yield (%)",xlab="Maturity (Years)", xlim=c(0,30),ylim=c(0,17))
opt=c(2:nrow(dyc)); for(i in 2:nrow(dyc)){lines(xm, dy[i,], lwd=1, type="b", lty=opt[i], col=opt[i]) }


## maturity in years and line type line
uy=matrix(NA,nrow(uyc), 11); for (j in 1: nrow(uyc)) {uy[j,]=as.numeric(uyc[j,1:11])}
plot(xm, uy[1,1:11], type='l', lty=1, lwd=1, col=1, ylab="Treasury Yield (%)",xlab="Maturity (Years)", xlim=c(0,30),ylim=c(0,17))
opt=c(2:nrow(uyc)); for(i in 2:nrow(uyc)){lines(xm,uy[i,], lwd=1, lty=opt[i], col=opt[i]) }

hy=matrix(NA,nrow(hyc), 11); for (j in 1: nrow(hyc)) {hy[j,]=as.numeric(hyc[j,1:11])}
plot(xm, hy[1,1:11], type="l", lwd=1, lty=1, col=1, ylab="Treasury Yield (%)",xlab="Maturity (Years)", xlim=c(0,30),ylim=c(0,17))
opt=c(2:nrow(hyc)); for(i in 2:nrow(hyc)){lines(xm,hy[i,1:11], lwd=1, lty=opt[i], col=opt[i]) }

fy=matrix(NA,nrow(fyc), 11); for (j in 1: nrow(fyc)) {fy[j,]=as.numeric(fyc[j,1:11])}
plot(xm,fy[1, ], type='l', lwd=1, col=1, ylab="Treasury Yield (%)",xlab="Maturity (Years)", xlim=c(0,30),ylim=c(0,17))
opt=c(2:nrow(fyc)); for(i in 2:nrow(fyc)){lines(xm,fy[i,], lwd=1, lty=opt[i], col=opt[i]) }

by=matrix(NA,nrow(byc), 11); for (j in 1: nrow(byc)) {by[j,]=as.numeric(byc[j,1:11])}
plot(xm, by[1, ], type='l', lty=1, lwd=1, col=1, ylab="Treasury Yield (%)",xlab="Maturity (Years)", xlim=c(0,30),ylim=c(0,17))
opt=c(2:nrow(byc)); for(i in 2:nrow(byc)){lines(xm, by[i,], lwd=1,lty=opt[i], col=opt[i]) }

dy=matrix(NA,nrow(dyc), 11); for (j in 1: nrow(dyc)) {dy[j,]=as.numeric(dyc[j,1:11])}
plot(xm, dy[1, ], type="l", lty=1, lwd=1, col=1, ylab="Treasury Yield (%)",xlab="Maturity (Years)", xlim=c(0,30),ylim=c(0,17))
opt=c(2:nrow(dyc)); for(i in 2:nrow(dyc)){lines(xm, dy[i,], lwd=1, lty=opt[i], col=opt[i]) }
